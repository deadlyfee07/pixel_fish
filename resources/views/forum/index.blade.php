...
@section('content')
<header class="mb-margin-lg flex justify-between items-center border-b-4 border-black pb-4">
    <div class="flex items-center gap-4">
        <span class="material-symbols-outlined text-[48px] text-primary">forum</span>
        <h1 class="font-headline-lg text-headline-lg text-primary uppercase drop-shadow-[4px_4px_0_#000]">STRATEGY TERMINAL</h1>
    </div>
    <a href="{{ route('forum.create') }}" class="bg-secondary text-black font-bold py-3 px-6 border-4 border-black shadow-[4px_4px_0_0_#000] hover:-translate-y-1 transition-all uppercase flex items-center gap-2 text-[10px]">
    ...
    @forelse($logs as $log)
        <article class="bg-surface border-4 border-black shadow-[8px_8px_0_0_#000] overflow-hidden relative group">
            <div class="bg-black p-4 flex justify-between items-center">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 bg-surface-container border-2 border-primary flex items-center justify-center">
                        <span class="material-symbols-outlined text-primary">person</span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-primary font-bold uppercase text-body-md">{{ $log->user->username }}</span>
                        <span class="text-zinc-500 text-[10px]">{{ $log->created_at->format('d M Y H:i') }}</span>
                    </div>
                </div>
                ...
                @if(auth()->user()->role === 'admin' || $log->user_id === auth()->id())
                    <form action="{{ route('forum.destroy', $log) }}" method="POST" onsubmit="return confirm('Hapus postingan ini?')">
                        @csrf @method('DELETE')
                        <button type="submit" class="text-zinc-500 hover:text-red-500 transition-colors p-2">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 bg-black/30 p-4 border-2 border-black">
                    <div>
                        ...
                        <p class="text-white font-bold">{{ $log->bait_type }} (x{{ $log->bait_amount }})</p>
                    </div>
                    <div>
                        <span class="text-primary text-[10px] uppercase font-bold">Yield (Fish):</span>
                        <p class="text-white font-bold">{{ $log->fish_caught }}</p>
                    </div>
                </div>
                <div class="mb-4">
                    <span class="text-[#ff6b6b] text-[10px] uppercase font-bold">Yield (Mats):</span>
                    <p class="text-zinc-300">{{ $log->ingredients ?? 'None' }}</p>
                </div>
                <div class="pt-4 border-t-2 border-dashed border-zinc-700 italic text-white/80">
                    "{{ $log->notes }}"
                </div>
            </div>
        </article>
