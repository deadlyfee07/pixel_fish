...
<section class="border-4 border-black shadow-[8px_8px_0_0_#000] bg-surface-container p-0 overflow-hidden flex flex-col">
    ...
    <div class="p-2 md:p-4 overflow-x-auto bg-[#121519]">
        <table class="w-full text-left font-body-md border-4 border-black border-collapse bg-surface">
            <thead class="bg-black text-[#a8e040] uppercase text-[9px] md:text-[10px] tracking-widest">
                <tr>
                    <th class="p-2 md:p-4 border-b-4 border-r-4 border-black w-1/5">ROD NAME</th>
                    <th class="p-2 md:p-4 border-b-4 border-r-4 border-black w-1/5">UPGRADE PATH</th>
                    <th class="p-2 md:p-4 border-b-4 border-r-4 border-black w-2/5">INGREDIENTS REQUIRED</th>
                    <th class="p-2 md:p-4 border-b-4 border-black w-1/5 text-right">GEM COST</th>
                </tr>
            </thead>
            <tbody class="text-[10px] md:text-[12px] font-bold uppercase">
                ...
                <tr class="border-b-4 border-black hover:bg-surface-container-high transition-colors">
                    <td class="p-2 md:p-4 border-r-4 border-black text-white break-words">{{ $rod->rod_name }}</td>
                    <td class="p-2 md:p-4 border-r-4 border-black text-[#97cbff] break-words">{{ $rod->upgrade_level }}</td>
                    <td class="p-2 md:p-4 border-r-4 border-black text-zinc-200 leading-relaxed break-words">{{ $rod->requirements }}</td>
                    <td class="p-2 md:p-4 border-black text-yellow-400 text-right text-[11px] md:text-[14px] whitespace-nowrap">{{ number_format($rod->gem_cost, 0, ',', '.') }} <span class="text-white text-[9px] md:text-[10px]">G</span></td>
                </tr>
                @endforeach
            </tbody>
        </table>
