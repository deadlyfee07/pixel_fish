...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    <div class="lg:col-span-1">
        <div class="bg-surface border-[4px] border-black {{ isset($bait) ? 'border-t-8 border-t-yellow-400' : '' }} shadow-[8px_8px_0_0_#000] p-6 sticky top-24 transition-all">
            ...
            <form action="{{ isset($bait) ? route('admin.baits.update', $bait) : route('admin.baits.store') }}" method="POST" class="flex flex-col gap-4">
                ...
                @if(isset($bait))
                    <div class="flex gap-2 mt-2">
                        <button type="submit" class="flex-1 bg-yellow-400 text-black font-bold border-[4px] border-black py-3 uppercase hover:-translate-y-1 hover:shadow-[4px_4px_0_0_#000] transition-all text-[10px]">UPDATE DATA</button>
                        <a href="{{ route('admin.baits') }}" class="bg-surface-container-high text-white font-bold border-[4px] border-black py-3 px-4 uppercase hover:bg-white hover:text-black transition-colors flex items-center justify-center text-[10px]">CANCEL</a>
                    </div>
                @else
                    <button type="submit" class="mt-2 bg-[#97cbff] text-black font-bold border-[4px] border-black py-3 uppercase hover:-translate-y-1 hover:shadow-[4px_4px_0_0_#000] transition-all text-[10px]">SAVE TO DATABANKS</button>
                @endif
            </form>
        </div>
    </div>
            ...
            <div class="p-4 overflow-x-auto">
                <table class="w-full text-left font-body-md border-4 border-black border-collapse bg-[#121519]">
                    <thead class="bg-black text-[#97cbff] uppercase text-[10px] tracking-widest">
                        <tr>
                            <th class="p-3 border-b-4 border-r-4 border-black">ITEM</th>
                            <th class="p-3 border-b-4 border-r-4 border-black">TARGETS</th>
                            <th class="p-3 border-b-4 border-black text-center w-24">ACTIONS</th>
                        </tr>
                    </thead>
                    <tbody class="text-[12px] font-bold uppercase">
                        ...
                        <tr class="border-b-4 border-black hover:bg-surface-container-high transition-colors">
                            <td class="p-3 border-r-4 border-black text-[#97cbff]">{{ $row->item_name }}</td>
                            <td class="p-3 border-r-4 border-black">
                                <div class="text-white">{{ $row->main_target }}</div>
                                <div class="text-zinc-500 text-[9px] mt-1">Bonus: {{ $row->bonus_target }}</div>
                            </td>
                            <td class="p-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.baits.edit', $row) }}" class="inline-block bg-yellow-400 text-black px-2 py-1 border-2 border-black hover:scale-110 transition-transform" title="Edit">
                                    ...
                                    <form action="{{ route('admin.baits.destroy', $row) }}" method="POST" onsubmit="return confirm('WARNING: Are you sure you want to delete {{ $row->item_name }}?');">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="inline-block bg-red-600 text-white px-2 py-1 border-2 border-black hover:scale-110 transition-transform" title="Delete">
